...
**Prepare an invoice** uses the same approved record for both GST/non-GST PDFs and Word documents. It requires an approved account. The employee cannot edit remittance fields there; the server ignores any client-supplied bank details. The section is labelled **Employee reimbursement remittance details**, not supplier payment details. The form lets the employee choose supplier signature or volunteer signature, without a separate signature-acknowledgment checkbox. For volunteer signing, the server uses the logged-in employee's identity, labels the output an **Expense Statement** (with GST details where applicable), and includes a volunteer expense declaration. The supplier non-GST registration declaration is hidden from the form, but included in generated non-GST PDF and Word documents only when the supplier signs. It is absent from volunteer-signed and GST documents. It never presents the volunteer as signing for the supplier. A volunteer-signed statement does not replace a supplier-issued GST tax invoice. Generating a document does not sign it or approve reimbursement; attach the signed document and available original bills. Receipt review and expenditure approval remain required.
