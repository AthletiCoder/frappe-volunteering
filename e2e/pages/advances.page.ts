...
export class AdvancesPage {
  ...
  heading() {
    return this.page.getByRole("heading", { name: "Advances", level: 1 });
  }
  ...
  managerFloatPanelHeading() {
    return this.page.getByRole("heading", {
      name: "Request from manager's advance",
      level: 2,
    });
  }
  ...
  teamFloatRequestsHeading() {
    return this.page.getByRole("heading", {
      name: "Team reimbursement requests",
      level: 2,
    });
  }
  ...
  managerFloatClaimLink() {
    return this.page.getByRole("link", {
      name: "New claim (manager's advance)",
    });
  }
  ...
  teamFloatRequestRow(claimName: string) {
    return this.page
      ...
      .filter({
        has: this.page.getByRole("link", { name: claimName, exact: true }),
      });
  }

  teamFloatRequestStatus(claimName: string) {
    return this.teamFloatRequestRow(claimName).getByText(
      /^(Can fund|Escalate)$/,
      { exact: true },
    );
  }
  ...
  async waitForTeamRequestsLoaded(): Promise<void> {
    await expect(this.page.getByText("Loading team requests…"))
