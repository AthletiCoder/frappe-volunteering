...
export class EmployeeAdvanceFormPage extends DeskForm {
  ...
  async openNew(): Promise<void> {
    ...
    await this.page.waitForFunction(
      () =>
        Boolean(
          (window as unknown as { cur_frm?: { doc?: unknown } }).cur_frm?.doc,
        ),
      undefined,
      { timeout: 45000 },
    );
  ...
  private async ensureSelfEmployee(): Promise<void> {
    await this.page.evaluate(async () => {
      ...
      if (!frm.doc?.employee) {
        const res = await win.frappe.db.get_value(
          "Employee",
          { user_id: win.frappe.session.user },
          "name",
        );
      ...
      if (frm.doc?.employee && !frm.doc?.company) {
        const empRes = await win.frappe.db.get_value(
          "Employee",
          frm.doc.employee,
          "company",
        );
        const company =
          typeof empRes?.message === "string"
            ? empRes.message
            : empRes?.message?.company;
        if (company) {
  ...
  async fillAdvance(
    amount: number,
    purpose = "E2E advance",
    populateRequestDefaults = true,
  ): Promise<void> {
    await this.ensureSelfEmployee();
    await this.page.evaluate(
      async ({ amt, text, populateDefaults }) => {
        const win = window as unknown as {
          cur_frm?: {
            doc: { intended_project?: string };
        ...
        if (populateDefaults && !frm.doc.intended_project) {
          const defaults = await win.frappe.xcall(
            "volunteering.volunteering.advance_portal.get_advance_request_form",
            {},
          );
          if (!defaults.projects.length)
            throw new Error(
              "Advance UI fixture needs an approved active project membership",
            );
          await frm.set_value("intended_project", defaults.projects[0].value);
          await frm.set_value("required_by_date", defaults.today);
          await frm.set_value("expected_settlement_date", defaults.today);
          await frm.set_value("advance_use", "My expenses");
        }
        await frm.set_value("purpose", text);
        await frm.set_value("advance_amount", amt);
      },
      { amt: amount, text: purpose, populateDefaults: populateRequestDefaults },
    );
  }

  async fillEmployeeAsAccounts(
  ...
  async open(name: string): Promise<void> {
    ...
    const docLoaded = await this.page
      .waitForFunction(
        (expected) =>
          (window as unknown as { cur_frm?: { doc?: { name?: string } } })
            .cur_frm?.doc?.name === expected,
        name,
        { timeout: 15000 },
      )
      .then(() => true)
    ...
    if (!docLoaded) {
      ...
      await this.page.waitForFunction(
        () =>
          Boolean(
            (window as unknown as { frappe?: { set_route?: unknown } }).frappe
              ?.set_route,
          ),
        undefined,
        { timeout: 45000 },
    ...
    await this.page.waitForFunction(
      (expected) =>
        (window as unknown as { cur_frm?: { doc?: { name?: string } } }).cur_frm
          ?.doc?.name === expected,
      name,
      { timeout: 45000 },
    );
    await this.page.waitForFunction(
      () =>
        document.querySelectorAll(
          ".form-layout .frappe-control, .form-page .frappe-control",
        ).length > 0,
      undefined,
      { timeout: 45000 },
    );
    ...
    await this.page.evaluate(() => {
      ...
      if (
        win.cur_frm &&
        win.volunteering?.accounting_workflow?.render_actions
      ) {
  ...
  async saveAndSubmit(options?: {
    expectReplenishWarning?: boolean;
  }): Promise<string> {
    const saveWait = this.waitForSaveResponse("Employee Advance");
