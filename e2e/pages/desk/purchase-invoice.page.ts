...
export class PurchaseInvoiceFormPage extends DeskForm {
  ...
  async createFromPo(poName: string): Promise<void> {
    ...
    await this.page.waitForFunction(
      () =>
        Boolean(
          (window as unknown as { frappe?: { db?: unknown } }).frappe?.db,
        ),
      undefined,
      { timeout: 45000 },
    );
    ...
    if (poDocstatus !== 1) {
      ...
      await this.page.waitForFunction(
        () =>
          Boolean(
            (window as unknown as { cur_frm?: { doc?: { items?: unknown[] } } })
    ...
    if (!mapped) {
      const needsManualBuild = await this.page.evaluate(
        async (purchaseOrder) => {
          const doc = await (
            window as unknown as {
              frappe: {
          ...
          const win = window as unknown as {
            ...
            frappe: {
              ...
              model: {
                sync: (
                  message: unknown,
                ) => Array<{ doctype: string; name: string }>;
              };
          ...
          win.frappe.call({
            ...
            callback: (response) => {
              ...
              if (!doc?.doctype || !doc?.name) {
                reject(
                  new Error("Purchase Invoice mapper returned no document"),
                );
                return;
              }
            ...
            error: (response) =>
              reject(
                new Error(
                  response.message ||
                    response.exc ||
                    "Purchase Invoice mapper failed",
                ),
              ),
          });
        }),
  ...
  private async buildPurchaseInvoiceFromPoInSession(
    poName: string,
  ): Promise<void> {
    await this.page.evaluate(async (purchaseOrder) => {
      ...
      const poDoc = await win.frappe.db.get_doc(
        "Purchase Order",
        purchaseOrder,
      );
      if (
  ...
  async markPaidOutside(remarks = "E2E cash to vendor"): Promise<void> {
    await this.page.evaluate(async () => {
      const frm = (
        window as unknown as {
          cur_frm?: { reload_doc?: () => Promise<unknown> };
        }
      ).cur_frm;
      if (frm?.reload_doc) {
    ...
    await this.page.waitForFunction(
      () => {
        const doc = (
          window as unknown as {
            cur_frm?: {
              doc?: { docstatus?: number; outstanding_amount?: number };
            };
          }
        ).cur_frm?.doc;
