...
export class DailyWorkLogFormPage extends DeskForm {
    ...
    await this.page.evaluate((emp) => {
      const frm = (
        window as unknown as {
          cur_frm?: { set_value: (f: string, v: string) => void };
        }
      ).cur_frm;
  ...
  async addItem(item: WorkLogItem & { skipProject?: boolean }): Promise<void> {
    ...
    if (!item.skipProject) {
      ...
      const option = this.page
        .getByRole("option")
        .filter({ hasNotText: /Advanced Search/ })
        .first();
      ...
      await this.page
        .waitForFunction(
          (expected) => {
            const rows =
              (
                window as unknown as {
                  cur_frm?: { doc?: { items?: Array<{ project?: string }> } };
                }
              ).cur_frm?.doc?.items || [];
            return rows.some((row) => row.project === expected);
          },
          item.project,
          { timeout: 15000 },
        )
        .catch(() => {
          throw new Error(
            `Daily Work Log fixture project could not be selected: ${item.project}`,
          );
        });
    }
    ...
    await grid
      .getByRole("textbox", { name: "Time Spent (Hours)" })
      .fill(String(item.hours));
  ...
  async expectWfhAutoApplied(): Promise<void> {
    await this.page.evaluate(() => {
      const frm = (
        window as unknown as { cur_frm?: { trigger?: (f: string) => void } }
      ).cur_frm;
      frm?.trigger?.("date");
    });
    await this.page.waitForFunction(
      () => {
        ...
        return /Work From Home|approved Attendance Request/i.test(
          desc?.textContent || "",
        );
      },
      undefined,
  ...
  async forceWfhFlag(checked: boolean): Promise<void> {
    await this.page.evaluate((value) => {
      const frm = (
        window as unknown as {
          cur_frm?: { set_value: (f: string, v: number) => void };
        }
      ).cur_frm;
