...
test.describe("HR Attendance @hr @ui", () => {
  ...
  test("HR-ATT-002 @regression @critical: Late work log corrects Absent to Present", async ({
    ...
    await expect
      .poll(
        async () =>
          (
            await e2eCall<{ status: string } | null>(
              request,
              "get_attendance_status",
              { employee: emp, date },
              "admin",
  ...
  test("HR-ATT-005 @regression @critical: Regularization locks status to Present", async ({
    ...
    await withPersona(browser, "employee", async (page) => {
      ...
      await reg.fillRequest({
        ...
        reason:
          "Forgot to log work; hours were actually completed for E2E test.",
      });
  ...
  test("HR-ATT-007 @regression: Manager rejects regularization", async ({
    browser,
    request,
  }) => {
    const cast = await getCast(request, "admin");
    ...
    await withPersona(browser, "employee", async (page) => {
      ...
      await reg.fillRequest({
        ...
        requestedStatus: "Present",
        reason:
          "Requesting Present after missed log; manager will reject in E2E.",
      });
      regName = await reg.saveDraft();
    });
  ...
  test("HR-ATT-009 @regression: Regularization beats approved leave", async ({
    browser,
    request,
  }) => {
    const cast = await getCast(request, "admin");
    ...
    await withPersona(browser, "employee", async (page) => {
      ...
      await reg.fillRequest({
        ...
        requestedStatus: "Present",
        reason:
          "Approved leave exists but regularization should lock Present status.",
      });
      regName = await reg.saveDraft();
    });
  ...
  test.describe("as employee", () => {
    ...
    test("HR-ATT-010 @regression: View own attendance list", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator(".list-row-head, .list-headers").first(),
      ).toBeVisible();
      await expect(
        page
          .locator(
            ".filter-section, .standard-filter-section, .filter-selector",
          )
          .first(),
      ).toBeVisible();
    });
