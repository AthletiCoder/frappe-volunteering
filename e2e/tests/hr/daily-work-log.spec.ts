...
test.describe("HR Daily Work Log @hr @ui", () => {
  test.describe("as employee", () => {
    ...
    test("HR-DWL-011 @regression: Employee can only create own work log", async ({
      ...
      await dwl.addItem({ project, hours: 6 });
      await dwl.save({
        expectError: /yourself|only create|not allowed|cannot|permission/i,
      });
    });
  });
  ...
  test("HR-DWL-012 @regression: Missing Daily Logs Report lists missing days", async ({
    page,
  }) => {
    const desk = new DeskForm(page);
    await desk.gotoReport("Missing Daily Logs Report");
    await expect(
      page.locator(".report-wrapper, .query-report, .dt-scrollable").first(),
    ).toBeVisible();
  });
});
