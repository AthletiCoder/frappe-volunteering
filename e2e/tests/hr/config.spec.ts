...
test.describe("HR Configuration & Settings @hr @ui", () => {
  ...
  test("HR-CFG-002 @regression @critical: Wrong Reports To routes approvals incorrectly", async ({
    ...
    try {
      ...
      if (approved !== 1) {
        await e2eCall(
          request,
          "seed_submit_attendance_request",
          { name: wfhName },
          "admin",
        );
      }
  ...
  test("HR-CFG-003 @regression: Designation set on Employee", async ({
    request,
  }) => {
    const cast = await getCast(request, "admin");
    const emp = cast.employee.employee!;
  ...
  test("HR-CFG-004 @regression: Department Head and Employee Department", async ({
    ...
    const cast = await getCast(request, "admin");
    const emp = cast.employee.employee!;
    const fixtures = await e2eCall<{ department: string }>(
      request,
      "get_fixtures",
      {},
      "admin",
  ...
  test.describe("as employee", () => {
    ...
    test("HR-CFG-008 @regression: Change backdate limit setting", async ({
      page,
      request,
    }) => {
  ...
  test("HR-CFG-009 @regression: Work log summary email preview", async ({
    browser,
  }) => {
    await withPersona(browser, "admin", async (page) => {
      const settings = new DailyWorkLogSettingsPage(page);
  ...
  test("HR-CFG-010 @regression: Leave Policy Settings page loads", async ({
    page,
  }) => {
