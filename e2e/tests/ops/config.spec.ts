...
test.describe("Ops email & digest config @ops @ui", () => {
  test.describe("as hr", () => {
    ...
    test("digest preview excludes unpaid staff from body", async ({
      request,
    }) => {
  ...
  test.describe("as admin", () => {
    ...
    test("Email Queue recent rows are readable @regression", async ({
      request,
    }) => {
    ...
    test("Daily Work Log Settings Preview Summary in UI @smoke", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator(".modal-dialog, .msgprint").first(),
      ).toBeVisible({
        timeout: 15000,
      });
    });
    ...
    test("Daily Work Log Settings form opens @smoke", async ({ page }) => {
      await page.goto(
        "/desk/daily-work-log-settings/Daily%20Work%20Log%20Settings",
        {
      ...
      await expect(
        page
          .locator(".layout-main, .page-container, #body, .desk-sidebar")
          .first(),
  ...
  test.describe("as employee", () => {
    ...
    test("employee cannot preview work log digest", async ({ request }) => {
      let denial = "";
      try {
        await e2eCall(request, "preview_work_log_digest", {}, "employee");
      } catch (error) {
        denial = String(error);
      }
      expect(denial.toLowerCase()).toMatch(/permission|not permitted|403/);
      expect(denial.toLowerCase()).not.toContain("csrftokenerror");
    });
  });
});
