...
test.describe("Accounts L1 smoke @smoke @accounts", () => {
  test.describe("as accounts", () => {
    ...
    test("AC-HOME-001: Accounts Home shows pay queues or empty Home", async ({
      page,
    }) => {
  ...
  test.describe("as employee", () => {
    ...
    test("AC-ADV-009: Advance Portal shows status", async ({ page }) => {
      ...
      await expect(
        emptyState.or(residualBadge.first()).or(loadError),
      ).toBeVisible();
    });

    test("AC-BKS-003: Home loads for employee spend actions", async ({
      page,
    }) => {
      const home = new HomePage(page);
      await home.goto();
