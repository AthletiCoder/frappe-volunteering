...
test.describe("Accounting settings @accounts @ui", () => {
  test.describe("as accounts", () => {
    ...
    test("AC-SET-005 @regression: Advances record an intended project without booking project spend", async ({
      page,
      request,
    }) => {
      ...
      const accountingProject = await e2eCall<string | null>(
        request,
        "get_doc_field",
        { doctype: "Employee Advance", name: advanceName, field: "project" },
        "accounts",
      );
      expect(accountingProject).toBeFalsy();
      await expect(
        page
          .locator(
            '.form-layout:visible [data-fieldname="project"], .form-page:visible [data-fieldname="project"]',
          )
          .first(),
      ).toBeHidden();
    });
