  ...
  test.describe("XM-003 @regression: Unpaid employee HR exclusions", () => {
    test("digest preview omits unpaid staff", async ({ browser, request }) => {
      ...
      await withPersona(browser, "hr", async (page) => {
        ...
        await expect(dialog.locator(".modal-body")).not.toContainText(
          employeeName,
        );
      });
    });
    ...
    test.describe("as unpaid", () => {
      ...
      test("advance is blocked before creation without an approved bank account", async ({
        page,
        request,
      }) => {
        const cast = await getCast(request, "unpaid");
        ...
        await advance.save({
          expectError: /Accounts Manager.*approve.*bank account/i,
        });

        const records = await getList(request, "Employee Advance", {
          fields: ["name"],
