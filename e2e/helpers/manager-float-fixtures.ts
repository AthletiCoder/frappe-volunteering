...
export async function seedManagerFloatClaim(
  ...
  const claim = await e2eCall<SeededExpenseClaim>(
    request,
    "seed_expense_claim",
    {
      employee,
  ...
  if (claim.workflow_state === "Pending Receipt Review") {
    const reviewed = await e2eCall<SeededExpenseClaim>(
      request,
      "seed_receipt_review",
      { name: claim.name },
      "admin",
...
export async function seedApproveExpenseClaim(
  ...
  if (state === "Pending Receipt Review") {
    await e2eCall(request, "seed_receipt_review", { name: claimName }, "admin");
  }
